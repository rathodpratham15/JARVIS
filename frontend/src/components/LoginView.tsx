...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess, initialMode = "login" }) => {
  ...
  // Handle SSO callback: /auth/callback?access_token=...&refresh_token=...
  useEffect(() => {
  ...
  const handleMicrosoftLogin = () => {
    window.location.href = `${BACKEND_URL}/auth/microsoft/login`;
  };
    ...
    <div className="min-h-screen bg-[#0d0f12] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-8">
        ...
        <div className="editorial-panel space-y-6">
          ...
          {/* SSO buttons — login only */}
          {isLogin && (config?.google_enabled || config?.microsoft_enabled || config?.saml_enabled) && (
